...
public sealed class TurbulenceDetectorTests
{
    ...
    public void Recent_authoritative_source_one_tier_gap_triggers_turbulence()
    {
        ...
        {
            new("alpha-3e", "p.001", Tier.T1, 2019, 6, 1.0, 0.79, null),
            new("nanda-3e",   "n.001", Tier.T1, 2010, 3, 1.0, 0.49, null),
            new("burstone",   "b.001", Tier.T1, 2015, 2, 1.0, 0.58, null)
        };
        var dissent = new Attestation("recent-2024", "r.001", Tier.T2, 2024, 1, 1.0, 0.87, null);

        var flag = TurbulenceDetector.Evaluate(dissent, consensus, Tier.T1);
    ...
    public void Old_low_weight_source_one_tier_gap_does_not_trigger()
    {
        ...
        var dissent = new Attestation("old-2005", "o.001", Tier.T2, 2005, 1, 1.0, 0.28, null);
    ...
    public void Old_source_two_tier_gap_triggers_due_to_lower_threshold()
    {
        ...
        var dissent = new Attestation("old-2005", "o.001", Tier.T3, 2005, 1, 1.0, 0.28, null);
    ...
    public void Promotion_detected_when_dissent_is_higher_tier()
    {
        var consensus = new Attestation[]
        {
            new("book-a", "a.001", Tier.T3, 2015, 1, 1.0, 0.50, null)
        };
        var dissent = new Attestation("book-b", "b.001", Tier.T1, 2024, 1, 1.0, 0.87, null);

        var flag = TurbulenceDetector.Evaluate(dissent, consensus, Tier.T3);
    ...
    public void Same_tier_never_triggers()
    {
        var consensus = new Attestation[]
        {
            new("book-a", "a.001", Tier.T1, 2019, 1, 1.0, 0.67, null)
        };
        var dissent = new Attestation("book-b", "b.001", Tier.T1, 2024, 1, 1.0, 0.87, null);
    ...
    {
        var dissent = new Attestation("book-a", "a.001", Tier.T2, 2024, 1, 1.0, 0.87, null);

        var flag = TurbulenceDetector.Evaluate(dissent, [], Tier.T1);
    ...
    public void Custom_tau_base_changes_threshold()
    {
        ...
        var dissent = new Attestation("book-b", "b.001", Tier.T2, 2015, 1, 1.0, 0.40, null);

        // Default τ=0.7: ratio 0.60 < 0.70 → no turbulence
        var flag1 = TurbulenceDetector.Evaluate(dissent, consensus, Tier.T1, tauBase: 0.7);
        flag1.Should().BeNull();

        // Lower τ=0.5: ratio 0.60 > 0.50 → turbulence
        var flag2 = TurbulenceDetector.Evaluate(dissent, consensus, Tier.T1, tauBase: 0.5);
    ...
    public void Three_tier_gap_has_very_low_threshold()
    {
        ...
        var dissent = new Attestation("book-b", "b.001", Tier.T4, 2010, 1, 1.0, 0.20, null);
